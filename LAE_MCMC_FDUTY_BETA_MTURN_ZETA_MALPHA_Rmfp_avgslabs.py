...
ACF_fiducial = (0.6228, 0.20785, 0.021)
ACF_error = (0.4, 0.12, 0.03)


#make an instance of this class with the loaded haloposition from 21cmFAST, in principle we only need to do this once for the entire scirpt
...
def lnlike(x, ACF_fiducial, density_of_observable_LAEs_afterEoR_fiducial, L_fiducial):
    ...
    ACF_model = np.zeros_like(acfbins)
    density_of_observable_LAEs_afterEoR = 0

    for sl in range(slabs):
        #these are the observable LAEs without reionization
        Observable_LAEs = LAE_positions.apply_parameters(intrinsic_params, LAEpos = LAE_slabs[str(7-sl)] )
        ...
        ACF_model += ACF_model_slab
    

    #average oibservables over slabs
    density_of_observable_LAEs_afterEoR =  density_of_observable_LAEs_afterEoR/float(slabs)
    ACF_model = ACF_model/float(slabs)
    ...
    #save chi2 informaiton
    chi2_acf.append(chi_squared_total)
    chi_squared_total += np.square(np.true_divide((density_of_observable_LAEs_afterEoR - density_of_observable_LAEs_afterEoR_fiducial), 9e-5 ))
    ...
    nmodel_list.append(density_of_observable_LAEs_afterEoR)
...
ACF_fiducial = (0.6228, 0.209, 0.021)
ACF_error = (0.4, 0.12, 0.03)
